...
function toDO($t) {
	...
	$pr = '\b(?:при|на|пере|по|под|за|м|из|в|от)?'; 		// приставки
	...
	////$t = preg_replace("~([".$s."])(\.) *$~ui", "$1ъ$2", $t); // перед точкой в конце текста
		...
		$t = preg_replace("/(глаг)ъ\./u", "$1.", $t);
		$t = preg_replace("/(гл)ъ\./u", "$1.", $t);
		$t = preg_replace("/(упо?трб?л?)ъ\./u", "$1.", $t);
		$t = preg_replace("/\b(пад)ъ\./u", "$1.", $t);
		$t = preg_replace("/\b(ред)ъ\./ui", "$1.", $t);
		$t = preg_replace("/\b(относящ)ъ\./u", "$1.", $t);
		$t = preg_replace("/\b(юж)н?ъ\./u", "$1.", $t);
		$t = preg_replace("/\b(ж|м|ср|мн)ъ\./u", "$1.", $t);
		$t = preg_replace("~\((\w?\w?[".$s."])ъ\)(\w)~u", "($1)$2", $t); // убирание ъ внутри закрывающей скобки
			$t = preg_replace("~ъ(\(|\)\w)~u", "$1", $t); // исключение: перед или в скобках посреди слов ъ не ставить
