...
function oformlenie($t) {
	...
	//$t = str_replace('||', '{{!}}{{!}}', $t); // внимание: меняет и внутри шаблонов
	...
	$t = preg_replace("~(?<![][{}<:.|/-])(\b[a-z]+[a-z ]+[a-z]+\b)(?![\}</])~ui", "{{lang|la|$1}}", $t); // {{lang}}
		$t = preg_replace("~\b([A-Z]\. +)(\{\{lang\|\w+\|)~u", "$2$1", $t);
		$t = preg_replace("~(\{\{lang\|\w+\|[^}]+)\}\}-(\b[a-z]+[a-z ]+[a-z]+\b)~ui", "$1-$2}}", $t);
		$t = preg_replace("~(\{\{lang\|\w+\|[^}]+)\}\}([-— ]+)\{\{lang\|\w+\|~ui", "$1$2", $t);
		$t = preg_replace("~(\{\{lang\|\w+\|[^}]*)\{\{lang\|\w+\|([^}]+)\}\}([^}]*\}\})~ui", "$1$2$3", $t); // уборка лишней простановки внутри других {{lang}}
		$t = preg_replace("~(\[(?:File|Файл):[^]]*)\{\{lang\|\w+\|([^}]+)\}\}~ui", "$1$2", $t); // уборка лишней простановки внутри [[Файл:]]
		$t = preg_replace("~\{\{lang\|\w+\|(nbsp)\}\}~ui", "$1", $t); // уборка лишней простановки внутри [[Файл:]]
		$t = preg_replace("~(section )\{\{lang\|la\|(begin|end)\}\}~ui", "$1$2", $t); // уборка лишней простановки
		$t = preg_replace("~\{\{lang\|la\|([XIV]+)\}\}~ui", "$1", $t); // уборка лишней простановки на рим.цифрах
