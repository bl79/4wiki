...
for v in volumes:
	...
	for article in v['wordlist']:
		subpagename = article[0]
		page_pn = article[1]
		section_name = article[2] if article[2] else subpagename
		...
		for section_text in p:
			...
			if page_pn == article[1]:	colontitul = ''  # на первых страницах без колонтитула
			else:			
				colontitul_center = subpagename.upper()
				colontitul_center = colontitul_center.replace(' (ПЛАТОН/КАРПОВ)', '').replace(' (ПСЕВДО-ПЛАТОН/КАРПОВ)', '')
