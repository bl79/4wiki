...
def make_pagetext(text, sectionname='', colontitul='', colontitul_up=True, header='', footer=''):
	if colontitul_up:
		if header == '':
			header = (f'<noinclude><pagequality level="1" user="TextworkerBot" /><div class="pagetext">'
					  f'__NOEDITSECTION__<div style="text-align:justify">{colontitul}<div class="indent"></noinclude>'
					  f'<section begin="{sectionname}" />')
		if footer == '':
			footer = '<section end="%s" /><noinclude><references /></div></div></div></noinclude>'
	else:
		if header == '':
			header = f'<noinclude><pagequality level="1" user="TextworkerBot" />' \
					 f'<div class="pagetext">__NOEDITSECTION__<div class="indent"></noinclude>'
					 # f'<div class="pagetext">__NOEDITSECTION__</noinclude><section begin="{sectionname}" />'
		if footer == '':
			# footer = f'<section end="{sectionname}" /><noinclude><references />{colontitul}</div></noinclude>'
			footer = f'<noinclude>\n<references />{colontitul}</div></div></noinclude>'
	# {{свр}}
...
def make_colontitul(book_pn, subpagename):
	...
	try:
		...
		pass
	colontitul_params = [
		label_interpages(book_pn_int, str(book_pn), ''), colontitul_center, label_interpages(book_pn_int, '', str(book_pn))
		# римские цифры
...
for v in volumes_from_file:
		...
		if i == 0:
			...
			if re.match('^\d+$', book_pn):
				...
				p = {'numbookpage': book_pn, 'numindexpage': scan_pn,
					 'colontitul': make_colontitul(book_pn, subpagename)}
				...
				if book_pn >= offset_pn_from and book_pn <= offset_pn_to:
					scan_pn = pagenum_to_scanpagenum(book_pn, page_scan_offset)
				p = {'numbookpage': book_pn, 'numindexpage': scan_pn,
					 'colontitul': make_colontitul(roman.toRoman(book_pn), subpagename)}
	...
	for t in volume_text_pages:
		...
		scanpagetext = make_pagetext(section_text, subpagename, colontitul, header='', footer='', colontitul_up=False)
	...
	vladi_commons.file_savetext(path + 'topost/' + str(v['filename']) + '.parsed.txt', '\n'.join(volume_text_to_post_pwb))
	pass
# import csv
#
