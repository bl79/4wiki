...
path = '/home/vladislav/var/tolstoy/html/'
volumes_from_file = [
	{
		'filename': 1,
		'index': 'L. N. Tolstoy. All in 90 volumes. Volume 56.pdf',
		'page_scan_offset': 33,
		'wordlist': [
...
for i in range(1, 92):
	...
	# re.sub('<h2><a href="#backn(\d+)">\1</a></h2>', '', text)
	...
	# text = re.sub('<span class="npdelimiter">([^<>]*)</span>', r'\1', text)


	text= re.sub('<?xml [^>]*>','', text)
	h = fromstring(text)
	h = h.cssselect('body')[0]

	# [h.remove(tag.getparent()) for tag in h.cssselect('h2 > a[href^="#back"]')]
	# [h.remove(tag.getparent()) for tag in h.cssselect('sup > a[href^="#back"]')]


	[e.drop_tag() for e in h.cssselect('span.opdelimiter')]
	[e.drop_tag() for e in h.cssselect('span.npdelimiter')]


	# for e in h.cssselect('a[name]'):
	# 	# e.attrib['pagenumber'] = e.attrib.pop('name', None)
	# 	e.tag = 'pagenumber'
	for e in h.cssselect('span.npnumber'):
		e.tag = 'pagenumber'
		e.attrib.pop('class', None)
		e.attrib['id'] = e.text
		e.text = None
	# [tag.getparent().remove(tag) for tag in h.cssselect('span[class="npnumber"]')]
	for e in h.cssselect('span.opnumber'):
		e.text = None
		e.drop_tag()
	# [tag.getparent().remove(tag) for tag in h.cssselect('span.opnumber')]

	# [tag.getparent().remove(tag) for tag in h.cssselect('a[name]') if not tag.text]
	for e in h.cssselect('a[name]'):
		e.drop_tag()
		pass
		# if not tag.text:
	...
	for e in h.cssselect('div.section[id~="h"]'):
		e.drop_tag()

	# удаление атрибута
	[e.attrib.pop('id', None) for e in h.cssselect('h1')]
	[e.attrib.pop('id', None) for e in h.cssselect('h2')]
	[e.attrib.pop('id', None) for e in h.cssselect('h3')]
	[e.attrib.pop('id', None) for e in h.cssselect('h4')]
	[e.attrib.pop('id', None) for e in h.cssselect('h5')]
	...
	print(i)
	# Сноски
	for ref in h.cssselect('a[type="note"][id]'):
		href = ref.get('href')
		if '#n' in href:
			id = href[href.find('#'):]
			# if id == 'n41':
			# 	pass
			cite = h.cssselect(id)[0]
			# try:
			# 	cite = h.cssselect(id)[0]
			# 	# cite = h.cssselect('div#' + id)[0]
			# except:
			# 	pass
			[sup.getparent().remove(sup) for sup in h.cssselect('sup > a[href*="#back"]')]
			[h.getparent().remove(h) for h in cite.cssselect('a[href*="#back"]')]
			etree.strip_tags(cite, 'h1', 'h2', 'h3', 'h4', 'h5')
			etree.strip_tags(cite, 'span', )
			atr_id = cite.attrib.pop('id', None)
			if atr_id:
				cite.attrib['name'] = atr_id
			cite.attrib.pop('class', None)
			ref.getparent().replace(ref, cite)
			cite.tag = 'ref'
			cite.tail = ref.tail
			if cite.text:
				cite.text = cite.text.strip()
			if len(cite) > 1:
				pass

			# [e.drop_tag() for e in ref.findall('./p[@class="left"]')]
			etree.strip_tags(cite, 'p')


			t = tostring(cite, encoding='unicode')
			pass

	# замена элемента на текст
	# vladi_commons.replace_element_to_text(h, 'br', r'\n')


	for e in h.cssselect('p.left'):
		e.attrib.clear()
		# try:
		# 	e.text = e.text.strip()
		# except:
	...
	[e.attrib.clear() for e in h.cssselect('p.forword')]
	[e.attrib.clear() for e in h.cssselect('p.avantitul')]

	for e in h.cssselect('p.center'):
		e.attrib.clear()
		e.tag = 'center'

	[e.drop_tag() for e in h.cssselect('div.image')]

	# for e in h.cssselect('div[class="section"]'):
	# 	e.attrib.clear()
	# 	e.tag = 'section'



	# for e in h.cssselect('sup'):
	# 	if e.text == None:
	# 		e.getparent().remove(e)
	[e.drop_tag() for e in h.findall('.//sup') if e.text == None]

	[e.drop_tag() for e in h.cssselect('div.body')]
	[e.drop_tag() for e in h.findall('.//div[@class="section"]')]
	[e.drop_tag() for e in h.findall('.//div[@class="forword"]')]
	[e.drop_tag() for e in h.findall('.//div[@class="avantitul"]')]
	# for e in h.findall('.//div[@class="avantitul"]')]
	#
	# etree.strip_tags(h, 'div')

	# удаление тегов без атрибутов
	[e.drop_tag() for e in h.cssselect('div') if not len(e.attrib)]

	etree.strip_tags(h, 'span')

	text = tostring(h, encoding='unicode')

	# text = re.sub('<br[\s/]*>', r'\n\n', text)
	# text = re.sub('<p>(.*?)</p>', r'\n\n\1', text, flags=re.DOTALL)
	# text = re.sub('<p class="center">(.*?)</p>', r'\n\n<center>\1</center>', text, flags=re.DOTALL)


	# text = re.sub('<br[ /]*>', '', text)

	text = re.sub("</strong><br/><strong>", "<br/>", text)
	text = re.sub("</em><br[\s/]*><em>", "<br/>", text)

	text = re.sub('</i>(\s*)<i[^>]*>', r"\1", text, flags=re.DOTALL)
	text = re.sub('</em>(\s*)<em[^>]*>', r"\1", text, flags=re.DOTALL)
	text = re.sub('</strong>(\s*)<strong[^>]*>', r"\1", text, flags=re.DOTALL)

	text = re.sub('<em[^>]*>(.*?)</em>', r"''\1''", text, flags=re.DOTALL)
	text = re.sub('<i[^>]*>(.*?)</i>', r"''\1''", text, flags=re.DOTALL)
	text = re.sub('<strong[^>]*>(.*?)</strong>', r"'''\1'''", text, flags=re.DOTALL)

	text = re.sub('<h1[^>]*>\s*(.*?)</?h1>', r'\n\n= \1 =\n', text)
	text = re.sub('<h2[^>]*>\s*(.*?)</?h2>', r'\n\n== \1 ==\n', text)
	text = re.sub('<h3[^>]*>\s*(.*?)</?h3>', r'\n\n=== \1 ===\n', text)
	text = re.sub('<h4[^>]*>\s*(.*?)</?h4>', r'\n\n==== \1 ====\n', text)
	text = re.sub('<h5[^>]*>\s*(.*?)</?h5>', r'\n\n===== \1 =====\n', text)

	text = re.sub("=\s*'''([^'].*?[^'])'''\s*=", r'= \1 =', text)
	text = re.sub("^(=+)([^=]*)<br[\s/]*>([^=]*)(=+)$", r'\1\2\4\n\n\1\3\4', text, flags=re.MULTILINE)
	text = re.sub("^(=+)([^=]*)<br[\s/]*>([^=]*)(=+)$", r'\1\2\4\n\n\1\3\4', text, flags=re.MULTILINE)
	text = re.sub("^(=+)([^=]*)<br[\s/]*>([^=]*)(=+)$", r'\1\2\4\n\n\1\3\4', text, flags=re.MULTILINE)
	text = re.sub("^(=+)([^=]*)<br[\s/]*>([^=]*)(=+)$", r'\1\2\4\n\n\1\3\4', text, flags=re.MULTILINE)
	text = re.sub("^(=+)([^=]*)<br[\s/]*>([^=]*)(=+)$", r'\1\2\4\n\n\1\3\4', text, flags=re.MULTILINE)
	text = re.sub("^=+\s*(.*?)\s*=+$", r'{{центр|\1}}', text, flags=re.MULTILINE)
	...
	text = re.sub(r'<p [^>]*class *= *[\'"]right[^>]*>(.*?)</p>', r'{{right|\1}}', text)
	text = text.replace('{{right2|}}', '')

	text = re.sub(r'</?body>', '', text)
	text = re.sub(r'(\[\[\s*\d+\s*\]\])', r'<nowiki>\1</nowiki>', text)


	text = re.sub(r'<center></center>', r'\n', text)
	# text = re.sub(r'\s*<center', r'\n<center', text)
	text = re.sub(r'<center>\s*(<pagenumber.*?/pagenumber>)\s*</center> *', r'\1', text)
	text = re.sub(r'(<p\b[^\s>]*>)\s*(<pagenumber.*?/pagenumber>)\s* *', r'\2\1', text)

	# text = text.replace('</pagenumber>', r'</pagenumber>\n')
	if re.search(r'</pagenumber>[^\s<]', text):
		pass
	...
	text = re.sub(r'\n{2,}', r'\n\n', text)
	# text = re.sub('^\s+$', r'', text, flags=re.MULTILINE)
	# text = re.sub(r'^\s+<', r'<', text, flags=re.MULTILINE)


	# text = re.sub(r'<ref name="(.*?)"[^>]*>\s*(.*?)\s*</ref>', r'@RRR@\1@\2RRR@', text)
	# text = re.sub(r'<pagenumber name="(.*?)"[^>]*></pagenumber>', r'@PPP@\1@PPP@', text)
...
#
# 			# colontitul = ''
# 			first_pages_without_colontitul = False  # на первых страницах без колонтитула
#
