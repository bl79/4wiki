...
//	preg_match_all("~(?<!tsdl\||</|<|\{\{)\b((?:[-\w]|\{\{[Аа]кут\d?\}\}|[́ˊ])+)\b(?!\.[\s{(']*(?:[".$abig."]|</small|tsdl))~u", $t, $words, PREG_OFFSET_CAPTURE); // сборка всех слов, кроме сокращений - вычисляются по наличию точки в конце и затем заглавной буквы или конца тэга
...
//	foreach($words as $w) $txt = str_replace($w, converter($w), $txt);
		...
		$t = preg_replace_callback("~(?<![[<{|/:])(".WORD.")(?!\}|\.[\s,;{()']*(?:[".A."]|</small|tsdl|tsdbr))~u",
			function ($word) {
				$w = $word[1];
				$stopwords = array('см','ср','глаг','гл','акут','Акут','Шейн', 'ипр');
				if (in_array($w, $stopwords)) return $w;
				foreach ($this->replaces_er_postfix as $substr => $r)
		...
		$t = preg_replace_callback("~(?<![[<{|/:])(".WORD.")~u",
			function ($word) {
				...
				if (in_array($w, $stopwords)) return $w;
				foreach ($this->replaces as $substr => $r)
					$w = preg_replace('~'.$substr.'~u', $r, $w);
				return $w;
			},
			$t);
